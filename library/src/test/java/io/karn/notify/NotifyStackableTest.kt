...
class NotifyStackableTest : NotifyTestBase() {
    ...
    fun multipleStackableTest() {
        ...
        Assert.assertEquals("2" + testSummaryTitle, notification.extras.getCharSequence(NotificationCompat.EXTRA_TITLE))
        ...
        Assert.assertEquals(testSummaryContent, NotifyExtender.getExtensions(notification.extras).getCharSequence(NotifyExtender.SUMMARY_CONTENT))
        Assert.assertEquals(Arrays.asList(testSummaryContent, testSummaryContent), notification.extras.getCharSequenceArray(NotificationCompat.EXTRA_TEXT_LINES).toList())
